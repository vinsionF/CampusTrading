...
function up_say(){
	...
	var text = $('.write_box input').val(),
		...
		str += '<div class="question_time">'+today.getHours()+':'+today.getMinutes()+':'+today.getSeconds()+'</div>'
	...
	if(text==''){
		...
		setTimeout(function(){
			...
			ans += '<div class="question_time" style="width:">'+today.getHours()+':'+today.getMinutes()+':'+today.getSeconds()+'</div>'
...
function autoWidth(){
	$('.question_text').css('ax-width',$('.question').width()-60);
}
